...
class SpaitalClustering:
    def __init__(self, sigma_rms_threshold=6.0, random_state=42, nsig=3.0, bic_max_components=5, min_cluster_size=2):
        ...
        self.gmm1 = GaussianMixture(n_components=1, random_state=self.random_state)
    ...
    def __call__(self, df, outpud_dir, cell_id):
        ...
        if np.sqrt(covs[0][0][0] + covs[0][1][1]) < self.sigma_rms_threshold:
            keep_mask = self._inlier_mask_mahalanobis(
                self.pts, labels, means, covs, nsig=self.nsig)
            ...
            keep_mask = self._select_cluster(labels, covs_n)
            df_filtered = df[keep_mask].copy()
            output_path = f"{outpud_dir}/cell_{cell_id}_spatial_clustering.png"
    ...
    def _inlier_mask_mahalanobis(self, pts, labels, means, covs, nsig=3.0, eps=1e-9):
        """
    ...
    def _select_cluster(self,labels, covs):
        """
        ...
        if covs.ndim == 3:            # full covariance (K, d, d)
            ...
            raise ValueError(f"Unexpected covs shape {covs.shape}; expected (K,d,d) or (K,d)")

        K = sigma_rms.shape[0]
        counts = np.bincount(labels, minlength=K)

        # Eligible clusters: sigma_rms below threshold and enough points
        eligible = (sigma_rms < float(self.sigma_rms_threshold)) & (counts > int(self.min_cluster_size))
